...
import kidridicarus.roles.robot.SMB.BounceCoin;
...
import kidridicarus.tiles.TileIDs;
import kidridicarus.worldrunner.WorldRunner;

/*
 * TODO:
 * -separate the bumpable item tile's body into it's own class so that bodies and roles are better separated
 *  (see how onBump ripples through the goomba body and then the goomba role, when the onBump shold go directly
 *  to goomba role
 */
public class BumpableItemTile extends BumpableTile {
	...
	private void startSpinningCoin(PlayerRole prHead) {
		...
		runner.givePlayerPoints(prHead, PointAmount.P200, true,
	...
	private void startBreakBrick(PlayerRole prHead) {
		...
		runner.givePlayerPoints(prHead, PointAmount.P200, false,
				body.getPosition(), GameInfo.P2M(GameInfo.TILEPIX_Y * 2), false);
