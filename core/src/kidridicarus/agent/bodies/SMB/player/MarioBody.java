...
public class MarioBody extends AgentBody {
	...
	private BlockingQueueList<AgentBody> headAgentContacts;
	...
	public MarioBody(Mario parent, Agency agency, Vector2 position, boolean isFacingRight, boolean isBig) {
		...
		headAgentContacts = new BlockingQueueList<AgentBody>();
	...
	private void processHeadContacts() {
		...
		if(canHeadBang && (b2body.getLinearVelocity().y > MIN_HEADBANG_VEL || prevVelocity.y > MIN_HEADBANG_VEL)) {
			...
			float closest = 0;
			AgentBody closestTile = null;
			for(AgentBody thingHit : headAgentContacts.getList()) {
	...
	/*
	 * Check the list of rooms that mario is currently in (or at least contacting) to find a room that contains the
	...
	public void onContactAgent(AgentBody agentBody) {
		...
		float agentY = agent.getPosition().y;

		// contact end of level flagpole?
		if(agent instanceof Flagpole)
			flagpoleContacted = (Flagpole) agent;
		else if(agent instanceof LevelEndTrigger) {
			levelendContacted = (LevelEndTrigger) agent;
			// the level end will trigger the castle flag
			levelendContacted.trigger();
		}
		// test for powerstar damage
		else if(agent instanceof DamageableAgent && parent.isPowerStarOn()) {
		...
		// does the agent do contact damage? (from non-head bounce source)
		else if(agent instanceof ContactDmgAgent && ((ContactDmgAgent) agent).isContactDamage()) {
