...
public class MarioSprite extends Sprite {
	...
	public void update(float delta, Vector2 position, MarioState agentState, MarioBodyState bodyState,
		...
		if(powerState != MarioPowerState.SMALL && !wasBig) {
		...
		else if(powerState == MarioPowerState.SMALL && wasBig) {
			if(runGrowAnimation)
				runGrowAnimation = false;
		...
		wasBig = (powerState != MarioPowerState.SMALL);
		...
		if(!isBigBody && (powerState != MarioPowerState.SMALL || runShrinkAnimation))
